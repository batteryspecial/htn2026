...
def crops_from(frame: np.ndarray, dets: sv.Detections, pad: float = 0.0) -> list[np.ndarray]:
...
class AttributeBank:
    ...
    def update(self, frame: np.ndarray, dets: sv.Detections, now: float) -> None:
        ...
        """
        if self.encoder is None or len(dets) == 0 or not (self._text or self._refs):
            return
        ids = dets.tracker_id
        if ids is None:
            return
    ...
    def matches(self, track_id: int, include, exclude, min_score: float) -> bool | None:
        """Does this track satisfy the subject's attribute rules?

        Returns None when the track has not been scored yet, which the caller
        treats as "not yet", not as "no": a behaviour should not fire on a
        track whose appearance is still unknown.
        """
        ...
        if entry is None or not entry.scores:
            return None
        if any(entry.scores.get(p, 0.0) < min_score for p in include):
            return False
        return not any(entry.scores.get(p, 0.0) >= min_score for p in exclude)

    def ref_match(self, track_id: int, reference_ids, min_sim: float) -> bool | None:
        """Does this track look like one of the uploaded reference photos?"""
        if not reference_ids:
            return True
        entry = self._cache.get(int(track_id))
        if entry is None or not entry.ref_sims:
            return None
        return any(entry.ref_sims.get(r, 0.0) >= min_sim for r in reference_ids)
